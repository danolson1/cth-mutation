...
def process_files(clcFilePath, identifiedBp = None):
    ...
    resultDict2 = get_sampleName_from_excel(resultDict, mTbl, filePath = inFilePath) # note 06-01-2017, this won't work because we haven't defined mTbl
...
def get_sampleName_from_excel(resultDict, mTbl, filePath = clcDataFilePath):
    """
    Inputs:
    ...
    for df in resultDict:
        resultDict[df] = resultDict[df].merge(lookupDf, how = 'left', left_on = 'Strain', right_on='Strain ID')
        resultDict[df].rename(columns = {'Filename' : 'Sample'}, inplace=True)
